...
MOVING_POSITION = [-1.69926117, 1.91009097, 2.09709026, -0.09968156, 1.5817661, 0.07704814, -0.375747]
...
class TychoEnv_rope(gym.Env):
    ...
    def __init__(self,
        ...
        if model_params_fn:
            self.load_model_params(model_params_fn)
    ...
    def create_action_space(cls, action_space, low=[-0.49,-0.20,0.1,0,-1,0,0,-0.6], high=[-0.46,-0.19,0.2,0,-1,0,0,-0.3]):
        if action_space == 'eepose':
            ...
            _high = np.array([0.2, 0.2, 0.2,0,0,0,0, 0.5])
            _low = np.array([-0.2,-0.2,-0.2,0,0,0,0,-0.5])
    ...
    def _set_action_space(self):
        ...
        self.action_space = TychoEnv_rope.create_action_space(self.config['action_space'],\
            low=self.config['action_low'], high=self.config['action_high'])

    def _set_observation_space(self):
        if not self.config['state_space'] in \
            ['eepose', 'jointpos','eepose-obj','jointpos-obj','eepose-obj-vel']:
    ...
    def gen_state(self, jointpos, eepose, objpos):
        if self._add_velocity:
            self.state_holder[:-11] = self.state_holder[-44:]
        if self._use_eepose_state:
            self.state_holder[-11:-3] = eepose
        else:
            self.state_holder[-10:-3] = jointpos
...
if __name__ == '__main__':
    ...
    env = TychoEnv_rope(onlyPosition=False)
